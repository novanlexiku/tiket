...
class M_laporan extends CI_Model{
	...
	function get_data_penumpang_pertanggal($tanggal){
		$hsl=$this->db->query("SELECT p.penumpang_tiket_seri,DATE_FORMAT(tanggal,'%d %M %Y') AS tanggal,p.penumpang_tiket_seri,p.penumpang_nama,p.penumpang_alamat,p.penumpang_usia,p.penumpang_jk,p.penumpang_kmp,k.kmp_id,k.kmp_nama,k.kmp_lintasan_id,l.lintasan_id,l.lintasan_nama,p.penumpang_tiket,p.penumpang_passport FROM tbl_penumpang p, tbl_kmp k, tbl_lintasan l WHERE DATE(tanggal)='$tanggal' and p.penumpang_kmp = k.kmp_id and k.kmp_lintasan_id = l.lintasan_id ORDER BY p.penumpang_tiket_seri ASC");
		return $hsl;
	}
